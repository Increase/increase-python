...
You can release to package managers by using [the `Publish PyPI` GitHub action](https://www.github.com/Increase/increase-python/actions/workflows/publish-pypi.yml). This authenticates with PyPI via OIDC trusted publishing, which requires a trusted publisher to be configured for the project on pypi.org.

### Publish manually

If you need to manually release a package, you can run `uv build` followed by `uv publish` with a PyPI
token for the project on the environment.
